...
curDate <- as.Date(Sys.Date())
url2 <- paste("https://www.ecdc.europa.eu/sites/default/files/documents/COVID-19-geographic-disbtribution-worldwide-" , curDate ,".xlsx", sep="")

# Interactive graph: hower to get date and figure

# Go back in time until we find something
while(GET(url2, write_disk(tf <- tempfile(fileext = ".xlsx")))$status_code != "200") {
  curDate <- curDate - 1
  url2 <- paste("https://www.ecdc.europa.eu/sites/default/files/documents/COVID-19-geographic-disbtribution-worldwide-" , curDate ,".xlsx", sep="")
}
...
covid_yu %>% 
  ...
  geom_line() + 
  xlab("") + ylab("") +
  labs (title = "F(x) in the last two weeks", 
        caption = "Data from: https://www.ecdc.europa.eu") +
  theme_minimal() +
  theme(plot.title = element_text(size = 14, vjust = 2, hjust=0.5),
        panel.grid.major.x = element_blank(),
...
covid_yu %>% 
  ...
  geom_line() + 
  xlab("") + ylab("") +
  labs (title = "logF(x) in the last two weeks", 
        caption = "Data from: https://www.ecdc.europa.eu") +
  theme_minimal() +
  theme(plot.title = element_text(size = 14, vjust = 2, hjust=0.5),
        panel.grid.major.x = element_blank(),
